...
void spellChecker::checkSentence(const string &sentence){
    ...
    while(streamedSentence >> word){
        ...
        misspelled.push_back(updatedWord);
    }
    if(misspelled.empty()){
...
void spellChecker::printRecommendedWords(vector<string> misspelled){
    ...
    for(int i = 0; i < misspelled.size(); i++){
        ...
        for(int j = 0; j < dictWords.size(); j++){
            ...
            //OPTIMIZATION!!!!! "one small line for programmer, one giant leap for program"-Hizbullah,2025
        ...
        for(int i = 0; i < dictWords.size(); i++){
            ...
            if(distance < minDistance){
                ...
                for(int k = 0; k < recommendations.size(); k++){
                    if(recommendations[k] == dictWords[i]){
                    alreadyAdded = true;
                    break;
                }
            }
...
void spellChecker::checkfile(const string& name) {
    ifstream file(name);
    if (!file.is_open()) {
        cout << "You messed up\n";
        return;
